...
import { deleteOrgToken } from "./actions";

export function OrgDeleteTokenButton({ tokenId, orgSlug }: { tokenId: string; orgSlug: string }) {
  const [pending, startTransition] = useTransition();

  function handleDelete() {
    startTransition(async () => {
      try {
        await deleteOrgToken(tokenId, orgSlug);
