...
import { createOrgToken } from "./actions";
...
export function OrgCreateTokenDialog({ repos, orgSlug }: { repos: Repo[]; orgSlug: string }) {
  ...
  const envLine = generatedToken ? `NEXT_PUBLIC_GLITCHGRAB_TOKEN=${generatedToken}` : "";
  ...
  function handleCreate() {
    ...
    startTransition(async () => {
      try {
        const token = await createOrgToken(selectedRepoId, name || "Default", orgSlug);
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="icon" onClick={() => handleCopy("token")} aria-label="Copy token">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="icon" onClick={() => handleCopy("env")} aria-label="Copy as .env line">
                  {copied === "env" ? <Check className="h-4 w-4 text-primary" /> : <Copy className="h-4 w-4" />}
                </Button>
              </div>
            </div>
            ...
            <Button onClick={() => handleClose(false)} className="w-full font-mono text-xs uppercase tracking-wider">
              Done
            </Button>
          </div>
        ) : (
            ...
            <div>
              ...
              >
                <PopoverTrigger className="flex w-full items-center justify-between gap-2 rounded-lg border border-input bg-transparent px-2.5 py-2 font-mono text-sm text-foreground transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                  <span className="truncate flex-1 text-left">{repoName || "Select a repo"}</span>
                  ...
                  <div className="max-h-48 overflow-y-auto p-1">
                    {filteredRepos.map((repo) => (
                      ...
                      >
                        ...
                        {repoName === repo.fullName && <Check className="h-3.5 w-3.5 text-primary shrink-0" />}
                      </button>
                    ))}
                    {filteredRepos.length === 0 && (
                      <p className="font-mono text-xs text-muted-foreground text-center py-3">No repos found</p>
                    )}
                  </div>
                </PopoverContent>
              </Popover>
